...
export function updateDragWaterInteraction(
  state: DragState,
...
export function emitDragReleaseRipple(state: DragState, time: number) {
...
export function updateRipples(delta: number) {
  ...
  for (let i = 0; i < maxRipples; i += 1) {
    ...
    if (ripple) {
      waterUniforms.uRippleCenters.value[i].set(
        ripple.center.x,
        ripple.center.y,
        ripple.direction.x,
        ripple.direction.y,
      );
      waterUniforms.uRippleData.value[i].set(ripple.age, ripple.lifetime, ripple.strength, ripple.shape);
    } else {
      waterUniforms.uRippleCenters.value[i].set(0, 0, 1, 0);
      waterUniforms.uRippleData.value[i].set(0, 1, 0, 0);
    }
  }
}
